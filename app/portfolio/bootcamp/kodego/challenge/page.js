...
import { getportfolioKodegoChallenges } from '@/sanity/sanity-utils';
...
export default function Portfolio() {
  ...
  const [portfolioKodegoChallenges, setPortfolioKodegoChallenges] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const portfolioKodegoChallengesData = await getportfolioKodegoChallenges();
        setPortfolioKodegoChallenges(portfolioKodegoChallengesData);
      } catch (error) {
  ...
  const openModal = (portfolioKodegoChallenge) => {
    setSelectedProject(portfolioKodegoChallenge);
    setModalOpen(true);
  };
    ...
    <div className={styles.container}>
        ...
        <div className={styles.CardInfoContainer}>
            ...
            <div className={styles.Cards + " mt-5 grid grid-cols-3 gap-8"}>
                {portfolioKodegoChallenges.map((portfolioKodegoChallenge) => (
                <div key={portfolioKodegoChallenge._id} className={styles.mainContainer}>
                    ...
                    <div className={styles.screen_container}>
                        ...
                        <div className={styles.card}>
                            ...
                            <div className={styles.imageContainer}>
                                {portfolioKodegoChallenge.image && (
                                <Image
                                    src={portfolioKodegoChallenge.image}
                                    alt={portfolioKodegoChallenge.name}
                                    fill={true}
                            ...
                            <div className={styles.textBox}>
                                <h1>{portfolioKodegoChallenge.name}</h1>
                                <button type="button" className={styles.portBtn} onClick={() => openModal(portfolioKodegoChallenge)}>
                                View
                                </button>
                            </div>
                            </div>
